...
const doRequest = (url, headers, requestOptions, limit) =>
  new Promise(function (resolve, reject) {
    ...
    req(
      Object.assign({ url, headers }, requestOptions),
      (error, response, body) => {
        if (error) {
...
function lookup(ids, idField, country, lang, requestOptions, limit, num) {
  idField = idField || "id";
  country = country || "us";
  num = num || 200;
  const langParam = lang ? `&lang=${lang}` : "";
  const joinedIds = ids.join(",");
  const url = `${LOOKUP_URL}?${idField}=${joinedIds}&country=${country}&entity=software${langParam}&limit=${num}`;
